\RequirePackage{amsmath}
\RequirePackage{ulem}
\newcommand{\VSTUInitializeCW} {
\renewcommand{\VSTUDocumentCodeSuffix}{81}
{
% Задание на курсовую работу
\clearpage
\thispagestyle{empty}
\begin{center}
\VSTUTitleHeading
\end{center}
Факультет~\uline{\VSTUFaculty\hfill}\\
Направление~\uline{\VSTUDirection\hfill}\\
Кафедра~\uline{\VSTUDepartment\hfill}\\
Дисциплина~\uline{\VSTUSubject\hfill}\\
\vfill
\hfill\VSTUTitleHeadApproval
\vspace{\fill}
\begin{center}
ЗАДАНИЕ\\
на курсовую работу
\end{center}
Студент $\underset{\text{(фамилия, имя, отчество)}}{\uline{\text{\VSTUStudentFullName}}}$\\
Группа \uline{\VSTUStudentGroup}\\
1. Тема: \VSTUTitleUL\uline{\hfill}\\
Утверждена приказом от <<\uline{\makebox[0.5cm]{}}>> \uline{\makebox[2cm]{}} \uline{\makebox[2cm]{}} \No\uline{\makebox[1.5cm]{}}\\
2. Срок представления готовой работы:~<<\makebox[1.5cm]{\hrulefill}>>\makebox[3.5cm]{\hrulefill}\the\year\\
3. Содержание расчетно--пояснительной записки:\\
\uline{\text{}\hfill}\\
4. Перечень графического материала:\\
\uline{\text{}\hfill}\\
5. Дата выдачи задания: <<\makebox[1.5cm]{\hrulefill}>>\makebox[3.5cm]{\hrulefill}\the\year\\
Руководитель работы $\underset{\text{(подпись и дата подписания)}}{\uline{\makebox[4.5cm]{\vphantom{\VSTUDirectorDegreeAndName}}}}$
\hfill
$\underset{\text{(инициалы и фамилия)}}{\uline{\makebox[6cm]{\VSTUDirectorDegreeAndName}}}$\\
Задание принял к исполнению $\underset{\text{(подпись и дата подписания)}}{\uline{\makebox[4.5cm]{\vphantom{\VSTUStudentName}}}}$
\hfill
$\underset{\text{(инициалы и фамилия)}}{\uline{\makebox[4cm]{\VSTUStudentName}}}$\\
\vspace{\fill}
\begin{center}
Волгоград,~\the\year
\end{center}
\newpage
% Титульник ПЗ
\clearpage
\thispagestyle{empty}
\setcounter{page}{1}
\begin{center}
\VSTUTitleHeading
\end{center}
Факультет~\uline{\VSTUFaculty\hfill}\\
Кафедра~\uline{\VSTUDepartment\hfill}\\
\vspace{2cm}
\begin{center}
ПОЯСНИТЕЛЬНАЯ ЗАПИСКА\\
\end{center}
по дисциплине \uline{\VSTUSubject\hfill}\\
на тему \VSTUTitleUL\uline{\hfill}\\
\\
\\
Студент $\underset{\text{(фамилия, имя, отчество)}}{\uline{\text{\VSTUStudentFullName}}}$\\
Группа \uline{\VSTUStudentGroup}\\
\\
\\
Руководитель работы $\underset{\text{(подпись и дата подписания)}}{\uline{\makebox[4.4cm]{\vphantom{\VSTUDirectorDegreeAndName}}}}$
\hfill
$\underset{\text{(инициалы и фамилия)}}{\uline{\makebox[6cm]{\VSTUDirectorDegreeAndName}}}$\\
Нормоконтролер $\underset{\text{(подпись и дата подписания)}}{\uline{\makebox[5.5cm]{\vphantom{\VSTUStandardsAdviserDegreeAndName}}}}$
\hfill
$\underset{\text{(инициалы и фамилия)}}{\uline{\makebox[6cm]{\VSTUStandardsAdviserDegreeAndName}}}$\\
\vspace{\fill}
\begin{center}
Волгоград,~\the\year
\end{center}
\newpage
}
}
